...
<div class="modal-overlay" id="settings-modal">
  <div class="modal">
    ...
    <div class="tab-content" id="tab-params">
      <div class="modal-section"><label>Mode Parameters (for selected mode)</label>
        <div class="param-grid">
          ...
          <div><label>Reasoning Budget</label><input type="number" id="p-reason" step="256" min="0" max="16384"></div>
        </div>
    ...
    <div class="tab-content" id="tab-agent">
      <div class="modal-section"><label>Agent Settings</label>
        <div class="param-grid">
          ...
          <div><label>Cmd Timeout (s)</label><input type="number" id="a-timeout" min="5" max="120"></div>
        </div>
        <div style="margin-top:8px">
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer"><input type="checkbox" id="a-verify"> Auto-verify (fix errors automatically)</label>
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer;margin-top:4px"><input type="checkbox" id="a-plan"> Plan first</label>
        </div>
...
function showPreview(html){codePanel.classList.add('open');codeViewer.style.display='none';previewFrame.style.display='block';previewFrame.srcdoc=html;}
...
document.getElementById('btn-settings').onclick=function(){
  fetch('/api/settings').then(function(r){return r.json();}).then(function(data){
    ...
    document.getElementById('a-plan').checked=ag.plan_first!==false;
...
document.getElementById('save-agent').onclick=function(){
  fetch('/api/agent/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({max_loops:parseInt(document.getElementById('a-loops').value),max_cmd_timeout:parseInt(document.getElementById('a-timeout').value),auto_verify:document.getElementById('a-verify').checked,plan_first:document.getElementById('a-plan').checked)})})
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/api/session/new': self._json({"id": app.new_session()})
            ...
            from synapse.core.config import save_context
            save_context(ctx)
            ...
            ctx["agent"]["plan_first"] = body.get("plan_first", True)
            from synapse.core.config import save_context
            save_context(ctx)
            self._json({"ok": True})
            ...
            try:
                ...
                if prov and prov in ctx.get("providers", {}):
                    if body.get("model"): ctx["providers"][prov]["model"] = body["model"]
                    if body.get("api_key"): ctx["providers"][prov]["api_key"] = body["api_key"]
                    from synapse.core.config import save_context
                    save_context(ctx)
