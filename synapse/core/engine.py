...
class Engine:
    ...
    def apply(self, name, providers):
        ...
        self.headers = {"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json", "User-Agent": "Synapse/4.0"}
        return True
    ...
    def set_params(self, params):
        ...
        self.reasoning_budget = params.get("reasoning_budget", 1024)
    ...
    def _stream_openai(self, messages):
        ...
        self.ctx_out = 0
        self.tokens_exact = False
        payload = {"model": self.model, "messages": messages, "stream": True, "temperature": self.temperature, "top_p": self.top_p, "max_tokens": self.max_tokens}
    ...
    def _stream_cohere(self, messages):
        ...
        payload = {"model": self.model, "message": last_msg.get("content", ""), "stream": True, "preamble": "\n".join(preamble_parts), "chat_history": chat_history, "temperature": self.temperature}
        req = urllib.request.Request(self.api_url, data=json.dumps(payload).encode("utf-8"), headers=self.headers, method="POST")
        try:
            with urllib.request.urlopen(req, timeout=120) as res:
                for line in res:
