...
.hdr-btn{width:30px;height:30px;border-radius:8px;background:var(--accent-dim);border:1px solid var(--border);color:var(--fg2);cursor:pointer;font-size:12px;display:flex;align-items:center;justify-content:center;transition:var(--tr)}
.hdr-btn:hover{background:var(--bg5)}
#main{flex:1;display:flex;overflow:hidden;position:relative}
#sidebar{width:230px;background:var(--bg2);border-right:1px solid var(--border);display:flex;flex-direction:column;transition:transform 0.3s;flex-shrink:0;z-index:15}
#sidebar.hidden{transform:translateX(-100%);position:absolute;height:100%}
.sb-hdr{padding:10px 12px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center}
.sb-hdr span{font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--fg3)}
.sb-btn{background:var(--accent);color:var(--bg);border:none;padding:3px 8px;border-radius:6px;cursor:pointer;font-size:10px;font-weight:600}
#session-list{flex:1;overflow-y:auto;padding:4px}
.sess-item{padding:8px 10px;border-radius:8px;cursor:pointer;margin-bottom:2px;display:flex;justify-content:space-between;align-items:center;font-size:12px;background:var(--bg3);border:1px solid transparent;transition:var(--tr);gap:4px}
.sess-item:hover{background:var(--bg4)}
...
.sess-btn{background:none;border:none;color:var(--fg4);cursor:pointer;font-size:10px;padding:2px 3px}
.sess-btn:hover{color:var(--fg)}
.sess-btn.del:hover{color:#f55}
#chat-area{flex:1;display:flex;flex-direction:column;overflow:hidden;max-width:860px;margin:0 auto;width:100%}
...
.msg{max-width:82%;padding:10px 14px;border-radius:var(--radius);font-size:13px;line-height:1.7;position:relative;animation:msgIn 0.3s cubic-bezier(0.4,0,0.2,1)}
...
.msg.thinking{align-self:flex-start;background:transparent;border:none;color:var(--fg4);font-size:11px;font-style:italic;padding:2px 14px}
.msg.err{align-self:flex-start;background:var(--bg3);border:1px solid #f44;color:#f55}
.msg.loop-status{align-self:center;background:var(--bg2);border:1px solid var(--border);color:var(--fg3);font-size:10px;padding:4px 12px;border-radius:16px}
.msg.cmd-result{align-self:flex-start;background:var(--bg);border:1px solid var(--border);font-family:monospace;font-size:11px;color:var(--fg2);white-space:pre-wrap;max-height:180px;overflow-y:auto}
.msg-actions{position:absolute;top:4px;right:4px;display:none;gap:3px}
.msg.user:hover .msg-actions{display:flex}
.edit-btn{background:var(--bg5);border:1px solid var(--border2);color:var(--fg2);border-radius:5px;padding:1px 6px;cursor:pointer;font-size:10px}
.md-content{white-space:pre-wrap;word-break:break-word}
.md-content strong{font-weight:700}
.md-content em{font-style:italic}
.md-content code{background:var(--bg);padding:1px 4px;border-radius:3px;font-family:monospace;font-size:12px;border:1px solid var(--border)}
.md-content h2,.md-content h3,.md-content h4{margin:6px 0 3px;font-weight:700}
.md-content ul{margin:3px 0 3px 16px}
.md-content li{margin:1px 0}
.md-content a{color:var(--fg);text-decoration:underline}
.code-block{margin:6px 0;border-radius:8px;overflow:hidden;border:1px solid var(--border);background:var(--bg)}
.code-header{background:var(--bg4);padding:4px 10px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border)}
.code-lang{font-size:9px;color:var(--fg3);font-weight:600;text-transform:uppercase;letter-spacing:1px}
.code-actions{display:flex;gap:3px}
.copy-btn,.preview-btn{background:var(--bg5);border:1px solid var(--border2);color:var(--fg2);padding:1px 8px;border-radius:5px;cursor:pointer;font-size:10px}
.code-content{background:var(--bg);padding:10px;overflow-x:auto;font-family:'SF Mono','Fira Code',monospace;font-size:12px;line-height:1.5;white-space:pre;color:var(--fg2)}
#mode-bar{padding:5px 20px;border-top:1px solid var(--border);display:flex;gap:5px;align-items:center;background:var(--bg2)}
.mode-btn{padding:4px 14px;border-radius:16px;border:1px solid var(--border);background:transparent;color:var(--fg3);cursor:pointer;font-size:10px;font-weight:600;transition:var(--tr)}
.mode-btn:hover{border-color:var(--fg4)}
.mode-btn.active{background:var(--accent);color:var(--bg);border-color:var(--accent)}
#mode-label{font-size:9px;color:var(--fg4);margin-right:3px;text-transform:uppercase;letter-spacing:1px}
#input-area{padding:10px 20px 14px;border-top:1px solid var(--border);display:flex;gap:8px;background:var(--bg2);flex-shrink:0;align-items:center}
#inp{flex:1;background:var(--bg3);border:1px solid var(--border);color:var(--fg);padding:10px 14px;border-radius:var(--radius);outline:none;font-size:13px}
#inp:focus{border-color:var(--fg4);box-shadow:0 0 0 2px var(--accent-dim)}
#send{background:var(--accent);color:var(--bg);border:none;padding:10px 18px;border-radius:var(--radius);cursor:pointer;font-weight:600;font-size:12px}
#send:disabled{opacity:0.3;cursor:default}
#stop{background:var(--bg4);color:var(--fg);border:1px solid var(--border2);padding:10px 18px;border-radius:var(--radius);cursor:pointer;font-weight:600;font-size:12px;display:none}
#edit-indicator{display:none;padding:4px 20px;background:var(--bg3);border-top:1px solid var(--border);font-size:11px;color:var(--fg3);align-items:center;justify-content:space-between}
#cancel-edit{background:none;border:none;color:var(--fg);cursor:pointer;font-size:11px;text-decoration:underline}
...
#code-panel.open{width:320px}
.cp-hdr{padding:8px 12px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center}
.cp-hdr span{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--fg3)}
.cp-close{background:none;border:none;color:var(--fg3);cursor:pointer;font-size:14px}
#code-file-list{padding:4px;border-bottom:1px solid var(--border);max-height:100px;overflow-y:auto}
.code-file-item{padding:5px 8px;border-radius:5px;cursor:pointer;font-size:11px;color:var(--fg2)}
.code-file-item:hover{background:var(--bg4)}
.code-file-item.active{background:var(--bg4);color:var(--fg)}
#code-viewer{flex:1;overflow:auto;padding:10px}
#code-viewer pre{font-family:monospace;font-size:11px;line-height:1.4;white-space:pre-wrap;color:var(--fg2)}
...
.modal{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:20px;width:92%;max-width:560px;max-height:85vh;overflow-y:auto;box-shadow:var(--shadow)}
.modal h3{font-size:14px;font-weight:700;margin-bottom:16px;letter-spacing:1px}
.modal-section{margin-bottom:14px}
.modal-section label{font-size:9px;color:var(--fg3);display:block;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;font-weight:600}
.modal textarea{width:100%;background:var(--bg);border:1px solid var(--border);color:var(--fg);padding:8px 10px;border-radius:8px;font-size:12px;min-height:60px;resize:vertical;font-family:inherit}
.modal input[type="text"],.modal input[type="password"],.modal input[type="number"],.modal select{width:100%;background:var(--bg);border:1px solid var(--border);color:var(--fg);padding:8px 10px;border-radius:8px;font-size:12px}
.modal select{cursor:pointer}
.modal-btns{display:flex;gap:6px;margin-top:16px;justify-content:flex-end}
.modal-btn{padding:6px 14px;border-radius:8px;cursor:pointer;font-size:11px;font-weight:600;border:none}
...
.param-grid label{font-size:9px;color:var(--fg3);text-transform:uppercase;letter-spacing:0.5px}
.param-grid input{margin-top:2px}
.theme-grid{display:flex;gap:6px;margin-bottom:8px}
.theme-opt{flex:1;padding:10px;border-radius:8px;border:2px solid var(--border);cursor:pointer;text-align:center;font-size:11px;font-weight:600}
...
.prov-badge{display:inline-block;padding:2px 6px;border-radius:5px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px}
...
.tab-btn{padding:6px 12px;border-radius:8px;border:1px solid var(--border);background:transparent;color:var(--fg3);cursor:pointer;font-size:11px;font-weight:600}
.tab-btn.active{background:var(--accent);color:var(--bg);border-color:var(--accent)}
.tab-content{display:none}
.tab-content.active{display:block}
@media(max-width:768px){
  #sidebar{position:absolute;z-index:15;height:100%;box-shadow:var(--shadow);width:250px}
  .msg{max-width:92%}
  #chat{padding:10px}
...
<div id="main">
  ...
  <div id="chat-area">
    <div id="chat"></div>
    <div id="edit-indicator"><span>Editing...</span><button id="cancel-edit">Cancel</button></div>
    <div id="mode-bar">
  ...
  <div id="code-panel">
    ...
    <div id="code-viewer"><pre>Click a file.</pre></div>
...
<div class="modal-overlay" id="settings-modal">
  <div class="modal">
    ...
    <div class="tab-content active" id="tab-general">
      <div class="modal-section"><label>Theme</label>
        <div class="theme-grid">
          <div class="theme-opt dark-opt" onclick="setTheme('dark')">Dark</div>
          <div class="theme-opt light-opt" onclick="setTheme('light')">Light</div>
        </div>
      </div>
    ...
    <div class="tab-content" id="tab-provider">
      <div class="modal-section"><label>Active Provider</label>
        <div id="prov-badge" class="prov-badge prov-nvidia">NVIDIA</div>
        <div class="provider-row"><select id="prov-select"></select></div>
        <div class="provider-row"><input type="text" id="model-input" placeholder="Model"></div>
        <div class="provider-row"><input type="password" id="apikey-input" placeholder="API Key"></div>
        <button class="modal-btn secondary" id="save-provider" style="width:100%;margin-top:4px">Save</button>
      </div>
    </div>
    <div class="tab-content" id="tab-params">
      <div class="modal-section"><label>Mode Parameters</label>
        <div class="param-grid">
          <div><label>Temperature</label><input type="number" id="p-temp" step="0.1" min="0" max="2"></div>
          <div><label>Top P</label><input type="number" id="p-topp" step="0.05" min="0" max="1"></div>
          <div><label>Max Tokens</label><input type="number" id="p-maxtok" step="256" min="256" max="32768"></div>
          <div><label>Reasoning</label><input type="number" id="p-reason" step="256" min="0" max="16384"></div>
        </div>
        <button class="modal-btn secondary" id="save-params" style="width:100%;margin-top:8px">Save Parameters</button>
      </div>
    </div>
    <div class="tab-content" id="tab-agent">
      <div class="modal-section"><label>Agent Settings</label>
        <div class="param-grid">
          <div><label>Max Loops</label><input type="number" id="a-loops" min="5" max="50"></div>
          <div><label>Cmd Timeout</label><input type="number" id="a-timeout" min="5" max="120"></div>
        </div>
        <div style="margin-top:8px">
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer"><input type="checkbox" id="a-verify"> Auto-verify errors</label>
        </div>
        <button class="modal-btn secondary" id="save-agent" style="width:100%;margin-top:8px">Save Agent Settings</button>
      </div>
    </div>
    <div class="modal-btns"><button class="modal-btn secondary" id="settings-close">Close</button></div>
  </div>
</div>
<script>
...
var busy=false,editIndex=-1,msgCount=0,renderQueue=0,currentSessionId=null,bgControllers={},currentMode='chat';
var PLACEHOLDERS={chat:'Sohbet et...',code:'Kod yaz...',agent:'Gorev ver...'};
function setTheme(t){document.documentElement.setAttribute('data-theme',t);localStorage.setItem('synapse-theme',t);}
(function(){var t=localStorage.getItem('synapse-theme')||'dark';setTheme(t);})();
tabBtns.forEach(function(b){b.onclick=function(){tabBtns.forEach(function(x){x.classList.remove('active');});document.querySelectorAll('.tab-content').forEach(function(x){x.classList.remove('active');});b.classList.add('active');document.getElementById('tab-'+b.getAttribute('data-tab')).classList.add('active');};});
...
function buildCodeBlock(p){
  ...
  if(p.lang==='html'||p.lang==='htm'){
    ...
    pvBtn.onclick=(function(code){return function(){codePanel.classList.add('open');codeViewer.style.display='none';previewFrame.style.display='block';previewFrame.srcdoc=code;}})(p.content);
...
function addMsg(role,text,idx){
  ...
  if(role==='user'){
    d.textContent=text||'';
    if(idx>=0){var acts=document.createElement('div');acts.className='msg-actions';var eb=document.createElement('button');eb.className='edit-btn';eb.textContent='Edit';eb.onclick=function(){editIndex=idx;inp.value=text;editInd.style.display='flex';inp.focus();};acts.appendChild(eb);d.appendChild(acts);}
...
modeBtns.forEach(function(b){
  b.onclick=function(){
    ...
    inp.placeholder=PLACEHOLDERS[currentMode]||'Message...';
    fetch('/api/mode',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:currentMode})});
  };
});
function sendMsg(){
...
function loadCodePanel(){
  fetch('/api/code_context').then(function(r){return r.json();}).then(function(data){
    ...
    if(!keys.length){codeFileList.innerHTML='<div style="padding:6px;font-size:11px;color:var(--fg4)">No files.</div>';return;}
...
document.getElementById('btn-settings').onclick=function(){
  fetch('/api/settings').then(function(r){return r.json();}).then(function(data){
    ...
    var mp=data.mode_params||{};
    document.getElementById('p-temp').value=mp.temperature||0.5;
    document.getElementById('p-topp').value=mp.top_p||0.9;
    document.getElementById('p-maxtok').value=mp.max_tokens||2048;
    document.getElementById('p-reason').value=mp.reasoning_budget||1024;
    var ag=data.agent_settings||{};
    document.getElementById('a-loops').value=ag.max_loops||25;
    document.getElementById('a-timeout').value=ag.max_cmd_timeout||30;
    document.getElementById('a-verify').checked=ag.auto_verify!==false;
    settingsModal.classList.add('show');
  });
};
document.getElementById('settings-close').onclick=function(){settingsModal.classList.remove('show');};
document.getElementById('save-params').onclick=function(){
  fetch('/api/params/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:currentMode,temperature:parseFloat(document.getElementById('p-temp').value),top_p:parseFloat(document.getElementById('p-topp').value),max_tokens:parseInt(document.getElementById('p-maxtok').value),reasoning_budget:parseInt(document.getElementById('p-reason').value)})})
  .then(function(){alert('Parameters saved.');});
};
document.getElementById('save-agent').onclick=function(){
  fetch('/api/agent/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({max_loops:parseInt(document.getElementById('a-loops').value),max_cmd_timeout:parseInt(document.getElementById('a-timeout').value),auto_verify:document.getElementById('a-verify').checked})})
  .then(function(){alert('Agent settings saved.');});
};
...
function loadSessions(){
  fetch('/api/sessions').then(function(r){return r.json();}).then(function(data){
    ...
    (data.sessions||[]).forEach(function(s){
      ...
      ren.onclick=function(ev){ev.stopPropagation();var nt=prompt('Rename:',s.title);if(nt&&nt.trim())fetch('/api/session/rename',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:s.id,title:nt.trim()})}).then(function(){loadSessions();});};
      ...
      del.onclick=function(ev){ev.stopPropagation();
        if(bgControllers[s.id]){bgControllers[s.id].abort();delete bgControllers[s.id];}
        fetch('/api/session/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:s.id})})
        .then(function(){if(s.id===currentSessionId){chat.innerHTML='';msgCount=0;currentSessionId=null;localStorage.setItem('synapse-session','');}loadSessions();});};
      acts.appendChild(ren);acts.appendChild(del);
      d.appendChild(t);d.appendChild(acts);
      d.onclick=function(){
        if(s.id===currentSessionId)return;
        currentSessionId=s.id;
        localStorage.setItem('synapse-session',s.id);
        chat.innerHTML='';msgCount=0;setBusy(false);
        fetch('/api/session/load',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:s.id})})
